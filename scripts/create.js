document.addEventListener("DOMContentLoaded", function() {
  ...
  if (createForm) {
    ...
    createForm.addEventListener("submit", async function(event) {
      ...
      const character = {
        ...
        tracos: [],
        // Adicionar inventário vazio ao personagem
        inventario: []
      };
      ...
      Array.from(tracosItems).forEach(tracoItem => {
        character.tracos.push({
          ...
          personalizado: tracoItem.dataset.personalizado === "true" // Indica se é um traço personalizado
        });
      });
      ...
      Array.from(tecnicasItems).forEach(tecnicaItem => {
        character.utensilio.tecnicas.push({
          ...
          personalizada: tecnicaItem.dataset.personalizada === "true" // Indica se é uma técnica personalizada
        });
      });

      // Coleta itens do inventário
      ...
      window.location.href = `view.html?index=${characters.length - 1}`;
    });
  }
  ...
  // Função para gerenciar o inventário
  function setupInventarioManager() {
  ...
  function addInventarioItem(item = {}) {
    ...
    // Cria os inputs para o nome, quantidade e descrição
    const nomeInput = document.createElement("input");
    nomeInput.type = "text";
    ...
    descricaoInput.placeholder = "Descrição do item";
    ...
    // Adiciona tudo ao elemento do item
    itemElement.appendChild(nomeInput);
    itemElement.appendChild(quantidadeInput);
    itemElement.appendChild(descricaoInput);
